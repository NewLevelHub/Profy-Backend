...
_INTELLIGENCE_RULES = """\
Правила для блока «Когнитивные навыки (учебные задания)» (intelligence):
- Это процент выполненных учебных заданий по каждому навыку, а не IQ, не \
уровень интеллекта и не норматив. Не используй слова «интеллект», «IQ», \
«СПН», «норма развития».
- Запрещены выводы про астенизацию, утомляемость, нервную систему, \
выносливость, работоспособность и любые диагнозы. Быстрые инструкции \
(quick_instructions) описывай только наблюдаемыми числами: верных ответов в \
первой и второй половине, ответов в лимит времени. При статусе \
insufficient_on_time не сравнивай половины.
- subject_profile — это осведомлённость в терминах предметных областей, а не \
способности. При status = mixed не называй ведущую область, при \
insufficient_data не интерпретируй профиль вовсе.
- Выводы о математических способностях делай только по числовым рядам \
(math_reasoning.numeric_series_percent); если divergence не none — назови \
расхождение между знанием терминов и решением числовых задач.
- Если protocol_quality.ok = false, прямо напиши, что результат \
интерпретируется с осторожностью, и назови причину по кодам флагов (ответы \
вне лимита времени, много пропусков, превышение времени субтеста, нет замера \
времени). legacy = true — попытка пройдена до обновления теста.
- Возраст и класс на момент прохождения — age_at_completion и \
grade_at_completion. У ученика младше 16 лет или ниже 10-го класса низкий \
результат в заданиях, опирающихся на школьные знания (осведомлённость, \
обобщение, числовые ряды), в первую очередь объясняй пройденной программой, \
а не способностями. Не делай вывода о неспособности к направлению по \
результату этого теста.
"""
...
def _system_prompt(context: PsychAiAnalysisContext) -> str:
    ...
    age_line = (
        f"Возраст ученика: {context.student_age}." if context.student_age is not None
        else "Возраст ученика неизвестен."
    )
    grade_line = f"Класс: {context.student_grade}." if context.student_grade is not None else "Класс неизвестен."
...
Контекст ученика: {age_line} {grade_line}
