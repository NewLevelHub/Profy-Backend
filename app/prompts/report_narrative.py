...
_AGE_STYLE = {
    AgeGroup.junior: (
        ...
        "никаких абстракций и терминов. Summary — минимум 3 коротких "
        "предложения (даже совсем короткие считаются — просто их должно "
        "быть не меньше трёх). Никаких профессий, карьеры, вуза, экзаменов — "
...
Структура ответа:
- summary: МИНИМУМ 3 предложения, максимум 5 (у junior тоже минимум 3, но \
короче по словам). Обязательно включи мысль в духе «это не окончательный \
выбор, а карта возможных направлений» — отдельным предложением, а не \
довеском к другому.
...
здесь evidence с source_type "thinking_style" или "motivation" — эти факты \
идут только в свои отдельные разделы (thinking_style_notes и \
motivation_narrative) со своей формулировкой; если продублировать их в \
strength_cards, ответ будет отклонён валидацией. title — короткая, конкретная \
...
профессий — это не career_narrative). Для junior: НИКАКИХ названий стилей \
и НИКАКИХ формулировок про работу/будущее/роли — только конкретное \
поведенческое описание («тебе легко...», «тебе нравится...»), без \
абстрактных ярлыков.
...
"thinking_style" или "motivation".
3. Количество strength_cards равно количеству evidence, у которых \
source_type НЕ "thinking_style" и НЕ "motivation" (в пределах 5-7, меньше — \
если фактов меньше 5).
...
5. summary — минимум 3 предложения (посчитай их сам перед отправкой).
