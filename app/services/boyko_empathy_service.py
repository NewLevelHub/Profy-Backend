...
async def answer_evidence(assessment_id: uuid.UUID, db: AsyncSession) -> dict[str, dict] | None:
    """Per-channel breakdown of the student's own Boyko answers, same shape
    and reasoning as eysenck_service.answer_evidence. `None` when nothing
    ...
    for order, text, value in rows:
        channel, _keyed = _ORDER_TO_KEY[order]
        answer = "yes" if value == _YES_VALUE else "no"
        entry = evidence.setdefault(channel, {"answered": 0, "yes": 0, "no": 0, "items": []})
        entry["answered"] += 1
        entry[answer] += 1
        entry["items"].append({
