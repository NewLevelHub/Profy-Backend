...
7 of its 8 subtests (subtest 8, «Геометрические фигуры», needs visual
stimulus material — Ф3.1, own separate ticket, excluded here by design).

Source: `Спецификации психологических тестов.docx`, section «Блок
«Характеристики интеллекта»» → «3. Полный список вопросов и варианты
ответов». That section's own header is explicit: **"официальный банк
заданий АСТУР (методическое руководство 1995 года) в открытом доступе не
найден... раздел 3 — рабочий черновик v1, составленный заново по формату и
духу методики... формулировки, сложность и ключи должны быть проверены и
...
98 items total = 90 scored (20+16+12+19+8+15, across 6 scored subtests) +
8 lability commands (own table, explicitly excluded from the total score
per the spec, see Ф3.5).
...
# metadata dict from SUBTESTS above. Deliberately NOT the array index of a
# possible future subtest 8 (geometric_figures) — when Ф3.1 lands, it slots
# in as number 8 without renumbering anything here.
...
assert _TOTAL_ITEMS == 98, f"АСТУР (7 subtests) must total 98 items, got {_TOTAL_ITEMS}"

_SCORED_TOTAL = sum(count for key, (_, count) in _EXPECTED_COUNTS.items() if key != "lability")
assert _SCORED_TOTAL == 90, f"scored items (excluding lability) must total 90, got {_SCORED_TOTAL}"
...
assert set(SUBTEST_BY_NUMBER.keys()) == {1, 2, 3, 4, 5, 6, 7}
