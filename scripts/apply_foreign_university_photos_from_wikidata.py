...
scripts/data/foreign_university_photo_review.json and uploads it via the
exact same pipeline as scripts/import_jinaq_university_photos.py
(app/services/university_photo_import_service.py) — these are foreign
universities jinaq doesn't cover at all, so they never got a photo any
other way.

Reuses import_university_photos() unchanged: creates a
UniversityExternalRef(source="wikidata", external_id=<Wikidata Q-id>) for
each confirmed entry first (same idempotency mechanism as the jinaq
pipeline — a re-run resolves straight to the existing ref instead of
re-matching), then feeds it a synthetic "institution" dict whose imageUrl
is a Special:FilePath link, resolved via Commons' own redirect chain
(Special:FilePath -> Special:Redirect -> upload.wikimedia.org) — hence
follow_redirects=True on the client, unlike the jinaq script which never
needed it.

Skips any entry with `confirmed` not true, or no `image_filename` — set by
scripts/generate_foreign_university_photo_review.py, reviewed by a human.

Run inside the api container:
  docker-compose exec api python scripts/apply_foreign_university_photos_from_wikidata.py [--dry-run] [--limit N]
"""
import argparse
import asyncio
import json
...
async def main(*, dry_run: bool, limit: int | None) -> None:
    ...
    async with async_session() as db:
        ...
        # Flushed (not committed) either way, so the lookup inside
        # import_university_photos sees these refs within this same
        # transaction even on a dry run; the whole transaction is rolled
        # back at the end if dry_run instead of skipping ref creation
        # entirely, which would make every entry resolve to "no_ref" and
        # dry-run nothing useful.
        await db.flush()
        ...
        if dry_run:
            # import_one_institution_photo never commits when dry_run=True,
            # so the refs flushed above are still just sitting in this open
            # transaction — roll it back explicitly rather than leaving a
            # dangling "idle in transaction" session.
            await db.rollback()
