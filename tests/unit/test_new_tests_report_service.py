...
def test_populated_containers_build_their_sections():
    analysis_result = _bare_analysis_result(
        professional_types={"scores": {"Ч-П": 5.0}, "top_type": "Ч-П"},
        eysenck={"extraversion": 12.0, "neuroticism": 8.0, "quadrant": "sanguine"},
    ...
    assert result.professional_types.top_type == "Ч-П"
    assert result.temperament.quadrant == "sanguine"
...
def test_one_malformed_container_does_not_blank_the_others():
    ...
    analysis_result = _bare_analysis_result(
        professional_types={"top_type": "Ч-Х"},
    ...
    assert result.professional_types.top_type == "Ч-Х"
