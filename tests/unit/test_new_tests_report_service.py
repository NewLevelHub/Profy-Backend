"""PRO-338 Ф0.2/Ф2.7 — isolation guarantee for the 6 new-tests specialist
...
async def test_no_data_yields_all_none_sections(db_session: AsyncSession):
    analysis_result = _bare_analysis_result()
    result = await build_new_tests_sections(
        analysis_result, assessment_id=analysis_result.assessment_id, db=db_session
    )
...
async def test_populated_containers_build_their_sections(db_session: AsyncSession):
    analysis_result = _bare_analysis_result(
...
async def test_one_malformed_container_does_not_blank_the_others(db_session: AsyncSession):
