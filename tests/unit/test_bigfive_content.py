...
cutoff) and `personality_notes()`, the tier-lookup that feeds
...
# --- personality_notes (tier -> wording table) -------------------

def test_covers_all_five_traits() -> None:
    notes = personality_notes(_MID_PROFILE)
    assert set(notes) == set(PERSONALITY_LABELS)


def test_high_relative_trait_gets_the_high_note() -> None:
    notes = personality_notes(_profile(openness=95.0, conscientiousness=5.0))
