"""Retake acceptance matrix: each real submit-answers entrypoint must —
through the shared `assessment_shared.invalidate_retake()` — delete the
stale, assessment-derived artifacts: the report row(s) + the report cache.

tests/integration/test_report_cache_resilience.py already pins the report
*cache key* being cleared; this file checks the row and the cache together
...
async def _assert_everything_gone(
    ...
    assert row is None, "report row survived retake"
