...
from app.services import assessment_shared, bigfive_service, riasec_service
...
async def test_new_instruments_do_not_change_riasec_or_bigfive_scoring(db_session: AsyncSession) -> None:
    ...
    riasec_before = await riasec_service.raw_scores(assessment.id, db_session)
    bigfive_before = await bigfive_service.raw_scores(assessment.id, db_session, mean_answer=4.0)
    ...
    riasec_after = await riasec_service.raw_scores(assessment.id, db_session)
    bigfive_after = await bigfive_service.raw_scores(assessment.id, db_session, mean_answer=4.0)
...
async def test_likert_total_questions_counts_new_instruments_automatically(db_session: AsyncSession) -> None:
    ...
    because `likert_total_questions` counts every row
    regardless of `instrument` — confirmed here rather than just asserted in
    the ticket."""
    await db_session.execute(delete(Question))
    before = await assessment_shared.likert_total_questions(db_session)
    assert before == 0

    await _seed_new_instrument_questions(db_session, n_per_instrument=7)

    after = await assessment_shared.likert_total_questions(db_session)
