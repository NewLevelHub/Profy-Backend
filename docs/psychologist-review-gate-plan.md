...
## Status: Implemented on `pro-337` (2026-09-14, обновлено 2026-09-15)
...
- `build_report()` сразу проставляет `pending_review` — шаги 1 и 3 раскатки §7
  сделаны в одной ветке, поэтому фронт с поддержкой pending-конверта должен
  выйти не позже бэкенда;
- гейт стоит не только на `/result`: все roadmap-ручки и уточнение
  направления тоже отвечают `409`, пока отчёт не опубликован (goal-context и
  gap-анализ, которые тоже были под гейтом, удалены в PRO-425);
- PATCH отклоняет (`422`) коды `strengths`/`weaknesses` не из профиля
  результата и `careers` не той формы;
- правка блока «Твой характер» хранится отдельно
  (`personality_notes_override`) и доходит до ученика — сохранённый
  `personality_notes` во взрослой формулировке ученику не показывается;
- с KZ-405 (своя строка отчёта на каждый язык) статус проверки один на
  прохождение: новая языковая строка наследует статус, правка психолога
  удаляет устаревшие переводы, публикация публикует все строки.

Исходный текст плана сохранён ниже.
...
> **PRO-425:** admin-заглушка ниже была реализована, но удалена — её заменил
> self-claim психолога (`GET /psychologist/students/available` →
> `POST /psychologist/students/{id}/claim`). Абзац оставлен как история.
...
(новый роут в `app/routers/admin.py` + сервис) — список `pending_review`
