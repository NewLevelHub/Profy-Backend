...
## Summary дублировал disclaimer — СДЕЛАНО (2026-08-13)

Пользователь заметил: LLM-summary часто заканчивался фразой «Это не
окончательный выбор, а карта возможных направлений» — а ровно эта же мысль
уже показывается отдельно как `disclaimer` прямо под summary на странице.
Промпт раньше ТРЕБОВАЛ эту фразу в summary (TZ_Profi.md §17.5 п.7) — теперь
наоборот явно запрещает её и любые вариации, требуя вместо неё содержательное
третье предложение. Добавлена жёсткая проверка `_check_summary_no_
disclaimer_duplicate` в валидаторе (одной смены промпта недостаточно — это
было mandatory-требование, для его отмены нужна активная проверка, а не
просто убранная инструкция). Fallback (`_summary()`) обновлён аналогично —
`_FRAME_PHRASE` убран, заменён на конкретное предложение с новым
содержанием.

223/223 тестов (было 207, включая независимо добавленные параллельной
работой — блок "Твой характер", кэп на onboarding-evidence уже с учётом
personality). Regression-proofed. Живая проверка: 3/3 LLM-сценария — ни
один не содержит запрещённую фразу.
...
## Новый блок "Твой характер" в `/result` — детерминированный, не через LLM (2026-08-13)

Пользователь заметил, что BigFive в проекте используется только частично:
`thinking_style` строится всего из 5 из 30 фасетов, а остальная часть
профиля (5 доменов N/E/O/A/C целиком) нигде не показывается ученику — хотя
BigFive прежде всего измеряет характер, а не только стиль мышления. Явный
запрос: добавить блок с характером для всех, кто проходит BigFive (все три
возраста — junior/middle/senior сдают BigFive одинаково, в отличие от
interest-инструмента), **не завязывая его чисто на LLM** и не ломая
архитектуру проекта.

**Решение** — детерминированный ассемблер-путь, как `interest_map`/
`careers`, а не narrative-пайплайн (LLM→валидатор→fallback), как
`summary`/`strength_cards`: одна и та же комбинация фактов всегда даёт один
и тот же текст, без риска LLM что-то придумать про характер ребёнка.

1. `bigfive_content.py`: добавлена `_NOTES_JUNIOR` — та же структура (5
   черт × 3 тира high/mid/low), что и взрослая `_NOTES`, но короче и
   конкретнее по формулировкам (junior — упрощённый текст, не тот же смысл
   более простыми словами методологически, а отдельная таблица).
   `personality_notes_for_age(is_junior, profile)` выбирает нужную таблицу.
2. `result_v2.py`: новая `StudentPersonalityNote {trait, label,
   description}`, поле `personality_notes: list[...]` с
   `min_length=max_length=5` на общей базе (`_ResultResponseBase`) — общее
   для MI и RIASEC веток, т.к. BigFive не зависит от interest-инструмента.
3. `report_narrative_context.py` / `report_narrative.py`: `"personality"`
   добавлен в `STRENGTH_CARD_EXCLUDED_SOURCE_TYPES` (по образцу
   `thinking_style`/`motivation`) — иначе высокотирные личностные черты
   попадали бы и в новый блок, и в `strength_cards` одновременно.
4. `report_v2_assembler.py`: `build_personality_notes()` — чистая функция
   от `personality_profile`, не трогает LLM.
5. `report_service.py`: подключено в оба пути — свежую генерацию
   (`build_report`) и реконструкцию из БД (`_shape_response`, читает уже
   сохранённый `AnalysisResult.personality_profile`, без новых колонок).

**Побочный фикс, найденный проактивно**: добавление нового *обязательного*
поля без дефолта означает, что любой уже закэшированный в Redis (TTL 24ч)
ответ, сгенерированный до деплоя, не пройдёт валидацию при чтении — раньше
это привело бы к 500. Исправлено `_cache_get_response()` — оборачивает
валидацию кэша в `try/except ValidationError`, при несовпадении схемы
падает обратно на БД (тот же принцип, что уже был у `_cache_get`/
`_cache_set` для `RedisError`: Redis — ускоритель, не источник истины).
Отдельный regression-тест (`test_report_cache_resilience.py`) кладёт в
Redis реальный ответ без `personality_notes` под текущим версионированным
ключом и проверяет, что `get_report` не падает, а пересобирает из БД.

Обновлены/добавлены: `test_bigfive_content.py` (новый, 8 тестов),
`test_report_v2_assembler.py`, `test_result_v2_schema.py`,
`test_report_narrative_validator.py`, `test_age_matrix_full_flow.py`, все
OpenAPI/example-снапшоты. Полный бэкенд-сьют: 222/222 зелёных.

Контракт (`frontend-result-api-contract.md`, корень `ProOf/`) обновлён —
новая §4.3a, `personality_notes` добавлен в список общих полей, §10
уточнён (student-facing `personality_notes` — не то же самое поле, что
одноимённый raw admin dict, тот же паттерн совпадения имён, что у
`strength_cards`/`thinking_style_notes`).

Фронтенд (`Profy-Frontend`): `StudentPersonalityNote` +
`personality_notes` в `shared/types/index.ts`, новый компонент
`PersonalitySection.tsx` (по образцу уже существующего
`StrengthCardsSection.tsx` — фиксированная сетка карточек, без
early-return на пустоту, т.к. поле всегда ровно 5 элементов) и подключён в
`ResultsPage.tsx` между картой интересов и стилем мышления. `npm run
typecheck` — 0 ошибок. Визуальная проверка в браузере не проводилась — в
этой сессии не было доступного инструмента для запуска/скриншота dev-
сервера, только typecheck и структурное соответствие уже задеплоенным
секциям с идентичным паттерном.

## Карточки "Подходящие профессии" — одинаковый `why` у всех карточек (2026-08-13)

Пользователь показал скриншот: у middle/senior все 5 карточек "Подходящие
профессии" (Архивариус/Аудитор/Бухгалтер/Директор по логистике/
HR-менеджер) показывали побайтово одинаковый текст `why`.

**Причина** (найдено чтением кода, не угадыванием): `career_match_score()`
(`riasec_service.py`) взвешивал только ранг буквы ПОЛЬЗОВАТЕЛЯ, а проверку
совпадения с кодом направления делал через `letter in direction_code` —
membership без учёта порядка букв в самом коде направления. `CSE`, `ESC`,
`SEC`, `CES` для одного пользователя набирали одинаковый счёт. Плюс
отягчающий фактор в данных: в `scripts/riasec_professions.py` (92
профессии) Архивариус/Аудитор/Бухгалтер имеют **буквально одинаковый** код
`"CSE"` — не анаграммы, а идентичная строка; кластер `{C,S,E}` — 13
профессий из 92, `{A,E,S}` — 22 из 92 (24%), т.е. случай системный, не
единичный.

Планировали и реализовали в 3 шага + собрали план негативного сценария
заранее (см. `/Users/arhat/.claude/plans/misty-swinging-lightning.md`),
прежде чем писать код:

1. **`career_match_score`**: заменён на позиционную конгруэнтность
   (Iachan-style — как в самой методологии Holland: совпадение с ПЕРВОЙ
   буквой кода направления весит больше, чем со второй/третьей). Новая
   `direction_letter_weight(letter, direction_code)` — 3/2/1/0 по позиции.
   Разные перестановки одного набора букв теперь дают разный счёт вместо
   плоской ничьи (шкала счёта поменялась 0-6 → 0-14, наружу не уходит,
   влияет только на сортировку внутри `matched_careers`).
2. **`matched_careers`**: дедуп по точному `holland_code` перед срезом по
   `limit` — если два направления буквально с одним и тем же кодом, в
   выдаче остаётся только победитель tie-break'а (по slug). Убирает
   тройной повтор Архивариус/Аудитор/Бухгалтер целиком: в топ-5 остаётся
   только одна из трёх. Существующий тест
   `test_matched_careers_tie_break_is_deterministic_by_slug` пришлось
   переписать — он specifically заводил два направления с одинаковым кодом
   и ожидал, что оба попадут в выдачу; теперь тест доказывает
   детерминированность tie-break'а через синтетический repeated-letter код
   (`"CCC"`/`"EEE"`, не встречается в реальных данных — ни один реальный
   код не повторяет букву), а отдельный новый тест проверяет сам дедуп.
3. **`_matched_strengths_for`** (`report_v2_assembler.py`): совпавшие
   evidence теперь сортируются по `direction_letter_weight` для КОНКРЕТНОГО
   направления (не по рангу пользователя) — ничего не убирается (все
   реально совпавшие факты остаются), меняется только порядок/акцент, так
   что `_join_ru` даёт разную фразу для разных перестановок одного набора
   букв.

**Проверка на реальных данных** (тот самый профиль `{C,S,E}` из
скриншота, через `docker exec` напрямую в `matched_careers`+
`build_riasec_careers`): тройной повтор исчез, счета стали разными
(14/13/13/13/11), но осталась **одна оставшаяся пара** с одинаковым `why`
— Финансовый аналитик (`CSI`) и Специалист техподдержки (`CSR`): коды
различаются только третьей буквой, которая не входит в топ-3 подтверждённых
букв пользователя, так что честно её и не за что процитировать — обе
карточки одинаково хорошо подтверждены ИМЕННО тем, что было точно
известно. Это ровно тот сценарий, что заранее описан в плане как триггер
для решения пользователя: донастроить алгоритм дальше или зафиксировать
как задачу по данным.

**Пользователь выбрал донастроить алгоритм.** Добавлен 4-й шаг —
same-response collision fallback в `build_riasec_careers`: если у второй
(и далее) показанной карточки совпадающий evidence-набор уже встречался
среди уже построенных карточек этого же ответа, к `why` добавляется одна
фраза с `skills_needed[0]` ЭТОГО направления — факт о профессии из
каталога, не утверждение о пользователе, поэтому не нарушает принцип
"только подтверждённые факты" (в отличие от того, чтобы процитировать
непроверенную букву RIASEC). Повторная проверка на том же реальном
профиле: все 5 карточек дали уникальный `why`.

Новые/обновлённые тесты: `test_riasec_service.py` (`direction_letter_weight`,
`career_match_score` — анаграммы дают разный счёт), `test_riasec_matched_
careers.py` (дедуп + переписанный tie-break тест), `test_report_v2_
assembler.py` (реордеринг даёт разный `why`; collision-fallback добавляет
skills_needed-клаузу). Полный бэкенд-сьют: 233/233 зелёных.

Известный оставшийся предел (не хак — честно задокументирован): если у
одного и того же направления И совпавший evidence-набор, И `skills_needed[0]`
случайно совпадут с уже показанной карточкой (крайне маловероятно, каталог
