...
Производные от отчёта ручки (roadmap, уточнение направления), пока отчёт на
...
- правка удаляет строки на других языках: это переводы текста до правки,
  ...
  статус;
- публикация публикует все оставшиеся языковые строки; правка и публикация
  берут ту же блокировку, что и генерация отчёта, поэтому перевод не может
  вклиниться между ними;
- переводится только повествование (сводка, итог, карточки сильных сторон и
  стиля мышления). Остальное, что правит психолог, в новую языковую строку
  **переносится** с проверяемой, а не пересчитывается по шкалам: коды
  сильных и слабых сторон — как есть; направления — в выбранном психологом
  составе и порядке, тексты из каталога на языке ученика, но любое поле
  направления, изменённое психологом (и добавленные им направления), —
  дословно; отредактированная мотивация и правка «Твой характер» — дословно,
  на языке, на котором их написали (перевести их нечем);
- если перевода не случилось (LLM выключен или все попытки не прошли) и
  новая строка получила детерминированный текст, отредактированные
  психологом поля повествования тоже переносятся дословно; неотредактированные
  остаются шаблонным текстом на языке ученика — как и машинный перевод, это
  текст, который психолог отдельно не читал;
- фразы «Твой характер» на странице проверки считаются на языке отчёта, а не
  на языке интерфейса психолога.
