...
полной выгрузки при обрезанных данных. На фронте: если словили 400 на этом
эндпоинте — показать `detail` как есть (там уже человекочитаемая просьба
сузить фильтры) и не ретраить без изменения фильтров.
