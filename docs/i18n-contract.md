...
профиля, если `users.locale` ещё дефолтный (`ru`, никем не менялся), он
предзаполняется эвристикой из `profiles.language`:

```
значение матчит /казах|qaz|қаз/i  →  users.locale = "kk"
иначе                             →  users.locale = "ru"
```

Пользователь может переопределить переключателем в любой момент.
