...
- **Остаётся на `ru` намеренно:** админка `/admin/*` (KZ-210); сырые данные
  ...
  **названия программ** (`Program.name`) — пока не в объёме перевода; `roadmap`
  и direction inquiry — dead code, не в проде (KZ-403/404); удалены в PRO-425.
...
|---|---|---|---|
| `riasec_question_bank` → `questions` | `order` (позиционный, 1..N) | `text`, `short_text` | `riasec_type`, `icon`, `order` |
| `bigfive_question_bank` → `questions` | `order` | `text` | `bigfive_domain`, `facet`, `keyed`, `order` |
...
## 14. Свободный текст внутри `requirements` — словарь по исходной строке

`*_i18n`-оверлей из §13 покрывает текст, у которого есть **своя колонка**
(`description`, `who_its_for`, `name`). Он не покрывает свободный текст,
лежащий внутри JSONB `Program.requirements`: `notes`, `exams`,
`source_required_documents`, `extracurriculars`, плюс `Program.language`,
`Program.career_options` и `name` / `conditions` каждой записи `Program.grants`. Именно из них собираются
карточки «Түсу талаптары» на странице программы — до этого они оставались
русскими под казахским заголовком.

- **Почему не `requirements_i18n`.** Симметричный оверлей на ряд был бы
  очевидным решением и здесь неверен: в каталоге ~12 434 программы, но всего
  ~4 611 **уникальных** фраз — одна и та же формулировка («Портфолио
  творческих работ», «Математика») повторяется у сотен программ. Поэтому
  перевод ключуется **исходной строкой**, а не рядом: одна запись обслуживает
  все программы, где эта фраза встречается.
- **Хранение — файл, не таблица.** `app/i18n/data/program_requirements_kk.json`,
  плоский `{ "<исходная строка>": "<kk>" }`, отсортирован по ключу (мерж даёт
  читаемый diff). Это статические справочные данные: едут в образе, ревьюятся
  в git как остальные контент-банки, миграции и seed-шага не требуют.
- **Read-side.** `app/i18n/data_strings.py`:
  `translate_data_string(text, locale=…)` / `translate_data_list(...)`. Матч
  точный, по обрезанной строке. Промах отдаёт исходную строку (никогда не
  пустоту и не ключ — §5) и тальится как `i18n.fallback`, чтобы дырка в
  покрытии была видна в метриках, а не молча рисовала русский текст. `ru` —
  сквозной проход. Файл читается один раз на локаль (`lru_cache`); отсутствие
  файла — легальное состояние, не ошибка.
- **Вызов — две точки, не одна.** `university_requirements.map_program_requirement`
  собирает `requirements_summary`. Но экран программы рисует **не только** его:
  `program.language`, `program.grants` и `program.career_options` он берёт из
  плоских полей `ProgramDetail` (а карточка в списке — из `ProgramBrief.language`).
  Поэтому те же поля переводятся ещё и в `university_service`
  (`get_program_detail` / `_program_brief` / `_localized_grants`). Локализовать
  только сводку недостаточно — под казахским заголовком оставался русский абзац
  про стипендию. Покрыто `tests/integration/test_university_locale.py`.
  Важно: `_note_hint_for_program` матчится по **исходным** `notes` (матчер
  опирается на русские слова-категории) и переводится уже результат — иначе на
  `kk`-запросе хинт перестал бы находиться.
- **Заполнение.** `scripts/apply_requirements_kk.py`: `dump` (непереведённые
  строки БД → `scripts/data/requirements_kk.todo.json`, сгруппированы по полю),
  `merge` (заполненный todo → стор, чистый JSON, БД не нужна), `stats`
  (покрытие по полям). `apply`-шага нет: стор и есть то, что читает рантайм,
  поэтому в runbook (`start.sh`, CD) ноль новых строк.
- **Гард мержа.** `is_plausible_translation` отсеивает значение, оставленное
  по-русски. Долю казахских букв считает только по **реально кириллическим**
  токенам — в каталоге полно турецких/немецких/испанских имён собственных
  (`Yabancı Uyruklu Öğrenci Sınavı`), и счёт «не-ASCII» и раздувал длину, и
  размывал долю. Дополнительные сигналы: казахские служебные слова
  (`және`, `немесе`, `бойынша`…) как признак перевода и русские служебные
  (`и`, `для`, `по`…) как признак его отсутствия — «Орта мектеп дипломы немесе
  баламасы» не содержит ни одной казахской буквы и обязано проходить.
  Совпадение с оригиналом разрешено, только если все кириллические слова с
  заглавной («Экономика: Математика + География.» в казахском такое же), и
  запрещено для строчных русских словоформ.
- **Покрытие.** 4 611 / 4 611 строк (100%) по всем полям на 2026-09-10;
  по фактическим вхождениям — 118 816 / 118 816. Первичный слой — LLM-качества,
  вычитка носителем по критичным формулировкам — за командой (см. §11).

## 15. CI-гарды локализации (KZ-602)
...
| 2026-09-24 | **PRO-425: MI, Harter-пары и возрастные тиры удалены.** Таблица `motivation_pairs`, колонки `questions.age_tier`/`mi_category`, `question_pairs.age_tier`, `motivation_statements.text_junior`, банки `mi_question_bank`/`motivation_pair_bank`/`question_pairing` и i18n-область `mi` удалены (миграция `d4a7c2e9f1b3`). Записи KZ-304/KZ-305 выше — история, описывают удалённый контент. |
